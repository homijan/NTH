...
\begin{frame}
\begin{center}
{\large Nonlocal Transport Hydrodynamic model}
...
\footnote{https://github.com/homijan/NTH-Laghos} 
\end{figure}
\end{center}
\end{frame}
...
\begin{frame}
\begin{center}
{\large Planar plastic target irradiated by NIF laser pulse}
\begin{figure}
\begin{center}
\begin{tabular}{ccc}
...
%\caption{.}
\label{RH_fig}
The~nonlocal transport of energy leads to generation of \textit{conduction zone} (turquoise color), whose thickness is one of the~main parameters for 
the~seeding of Rayleigh-Taylor and Richtmyer-Meshkov instabilities.
\end{center}
\end{figure}
\footnotetext{M. Holec, J. Nikl, and S. Weber, "Nonlocal Transport Hydrodynamic Model for Laser Heated Plasmas", \textit{Phys. Plasmas}, 
submitted (2017).}
...
matter", \textit{Phys. Rev. Lett.}, 120, 025002 (2018).}
\end{frame}
...
\begin{frame}
\begin{center}
\myheading{Example of moment methods}
\begin{block}{AWBS Boltzmann transport equation}
\begin{equation}
  \vn\cdot\nabla f + \frac{1}{\vmag}\left( \E + 
  \vmag\vn\times\B\right)\cdot\nabla_{\vect{v}} f = 
  \frac{\vmag}{\lambda} 
  \pdv{}{\vmag}\left( f - \fM\right) + \frac{1}{2 \lambda_{ei}}
  \pdv{}{\mu}\left((1-\mu^2)\pdv{f}{\mu} \right).
  \nonumber\label{eq:AWBS}
\end{equation}
\end{block}
\begin{block}{M1-AWBS transport model}
...
\end{eqnarray}
The~second moment closure uses the~M1 form $\matr{A}(\fzero, \fone)$, i.e. is
nonlinear and degenerates to $\matr{I}\frac{1}{3}$ with decreasing anisotropy.
\end{block}

{\large High-order finite element bilienar form integrators}
\begin{equation}
...
\begin{frame}
\begin{center}
{\large Nonlocal Transport Magneto-Hydrodynamic model}  
\begin{equation}
  %\frac{1}{|\vect{v}|}\frac{\partial f}{\partial t} + 
  \vect{n}\cdot\nabla_{\vect{x}} f + \frac{1}{|\vect{v}|}
  \left( \vect{E} + \vect{v}\times\vect{B}\right)\cdot\nabla_{\vect{v}} f
  = 
  \frac{f_{MB}(|\vect{v}|, T_e) - f}{\lambda_{ei}(|\vect{v}|^4)} \rightarrow
  \matr{D}\cdot\vect{f} + \frac{1}{|\vect{v}|} 
  \vect{E}\cdot\matr{V}_E\cdot\vect{f} + \matr{B}\times\matr{V}_B\cdot\vect{f}
  = 
  \vect{S} - k_{|\vect{v}|}\matr{M}\cdot\vect{f}
  \nonumber
\end{equation}
...
\begin{eqnarray}
  ...
  \frac{\vect{j}}{e n_e \sigma}\qquad\quad +
  \qquad \frac{1}{e n_e c}\vect{j}\times\vect{B}
  \nonumber \\
...
\begin{equation}
  ...
  {\sum_{\Delta v^g} \int_{4\pi}\matr{V}_E\cdot\vect{f}}  
...
\begin{equation}
  ...
  \left( \vect{E} + \vect{v}\times\vect{B}\right)\cdot\nabla_{\vect{v}} f^e
  = 
  %n_i \frac{\dI \bar{Z}}{\dI t} \frac{3}{8\pi} + 
  \frac{k_B n_e\frac{\sqrt{2}}{\pi^{\frac{3}{2}} }v_{th} T_e ~-~ f^e}
  {\lambda^{e}} 
  %\sqrt{\frac{2 k_B T_e}{m_e}}\right) T_e\\ 
  %\sigma\left(\frac{\dI \bar{Z}}{\dI t}, \rho, T_e\right) T_e
  %\, -\, \frac{I^e}{8.51 \lambda^{ei}} 
...
\begin{equation}
  %\varepsilon^e &=& \frac{1}{\bar{|\vect{v}|}}\int_{4\pi} I^e\, \dI\vect{n}\, 
  %\nonumber \\ 
  \vect{q}_e = \int_{4\pi}\vect{n}\, f^e\, \dI\vect{n}\, 
  \xrightarrow{diffusive} \nabla\cdot\vect{q}_e = 
  - \nabla\cdot\left(\kappa_{SH}\, T_e^{\frac{5}{2}}\, \nabla T_e\right) 
  \nonumber\label{ele_momentum} 
  %\\
...
\begin{itemize}
\item PETE simulations: Lagrangian frame, 2T single fluid, IB laser deposition, SESAME
...
\item High-order finite element scheme for 6D nonlocal radiation and electron transport
...
\item What about a~Multi-Grid approach for an~efficient solution of DG-BGK$\&$T?
\item What about an~implementation of the~spherical harmonics basis in MFEM?
 
\end{itemize}
\begin{figure}
\includegraphics[width=0.5\textwidth]{../figs_NTH/nth_sedov.png}
\end{figure}
%\begin{figure}
%\begin{center}
%\begin{tabular}{ccc} 
